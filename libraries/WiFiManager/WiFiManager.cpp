...
void WiFiManager::setupConfigPortal() {
  ...
  DEBUG_WM(F("Dang cau hinh diem truy cap ... "));
  DEBUG_WM(_apName);
  if (_apPassword != NULL) {
    if (strlen(_apPassword) < 8 || strlen(_apPassword) > 63) {
      // fail passphrase to short or long!
      DEBUG_WM(F("Mat khau diem truy cap khong hop le, bo qua."));
      _apPassword = NULL;
    }
  ...
  DEBUG_WM(F("HTTP server da duoc khoi tao"));
}
  ...
  while(1){
    ...
    if (connect) {
      ...
      if (connectWifi(_ssid, _pass) != WL_CONNECTED) {
        DEBUG_WM(F("Ket noi khong thanh cong."));
      } else {
        //connected
        WiFi.mode(WIFI_STA);
...
int WiFiManager::connectWifi(String ssid, String pass) {
  DEBUG_WM(F("Dang ket noi nhu WIFI may khach ..."));
  ...
  if (WiFi.status() == WL_CONNECTED && (WiFi.SSID() == ssid)) {
    DEBUG_WM(F("Da san sang ket noi ..."));
    return WL_CONNECTED;
  }
  ...
  if (ssid != "") {
    ...
    if (WiFi.SSID() != "") {
      DEBUG_WM(F("Su dung cau hinh duoc luu lan cuoi."));
      ...
      DEBUG_WM(F("Khong co thong tin duoc luu."));
    }
  }

  int connRes = waitForConnectResult();
  DEBUG_WM ("Ket qua ket noi: ");
  DEBUG_WM ( connRes );
...
uint8_t WiFiManager::waitForConnectResult() {
  if (_connectTimeout == 0) {
    ...
    DEBUG_WM (F("Cho ket qua ket noi"));
    ...
    while (keepConnecting) {
      ...
      if (millis() > start + _connectTimeout) {
        ...
        DEBUG_WM (F("Qua thoi gian ket noi"));
      }
...
void WiFiManager::handleReset() {
  ...
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += F("Module se khoi dong lai trong vai giay.");
  page += FPSTR(HTTP_END);

  server->sendHeader("Content-Length", String(page.length()));
  server->send(200, "text/html", page);

  DEBUG_WM(F("Lam moi trang"));
  delay(5000);
  ESP.reset();
  delay(2000);
}
